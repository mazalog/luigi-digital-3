...
function ShoppingCartIcon(props) {
    ...
    return (
        <div className={`shopping-cart-icon ${props.className}`} onClick={() => { props.setActive(props) }}>
            <div className="cart-icon" style={{ position: "absolute", }}>
                <BagIcon fill="#fff" />
            </div>
            <span 

            className={`${quantityCounter > 0 ? `quantity-counter ${props.right}` : null}`}>{quantityCounter > 0 ? quantityCounter : null}</span>
        </div>
    )
}
